...
def a_star(start_puzzle):
    ...
    heapq.heappush(frontier, (start_puzzle.manhattan_distance(), 0, next(counter), start_puzzle, []))
    visited.add(tuple(start_puzzle.to_array()))

    while frontier:
        est_total_cost, cost_so_far, _, current, path = heapq.heappop(frontier)
        ...
        for move in current.get_legal_moves():
            ...
            if state_key not in visited:
                ...
                heapq.heappush(frontier, (est_cost, new_cost, next(counter), next_state, path + [move]))
